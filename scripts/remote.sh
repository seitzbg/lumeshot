...
case "$cmd" in
  ...
  run)    ssh "$MAC_HOST" "cd $MAC_DIR && swift build -c release 2>&1 && scripts/bundle.sh && { pkill -f 'Lumeshot.app/Contents/MacOS/SXApp' 2>/dev/null; sleep 1; pkill -9 -f 'Lumeshot.app/Contents/MacOS/SXApp' 2>/dev/null; sleep 1; open \"dist/Lumeshot.app\" --args $*; }" ;;
