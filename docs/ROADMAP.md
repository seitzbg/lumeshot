...
- The editor window is sized to the capture: the image opens at 1:1 (its pixels ÷ the
  display's backing scale, plus the tool rail and top bar), capped to 90% of the visible
  frame for captures larger than the screen, floored at 760×480 for small ones. Replaces
  the remembered-size behaviour from v0.1.21. The window stays resizable. Not yet
  re-checked in a signed-build smoke.
...
- The annotation editor's drawing tools moved from an icon-only top row into a
  labelled vertical rail on the left (icon + name, active tool highlighted), so each
  tool's function is obvious without hovering for a tooltip. The top bar keeps colour,
  width, undo/redo/delete and the finish buttons, and the editor window opens compact
  again. Not yet re-checked in a signed-build smoke.
...
[v0.1.13](https://github.com/seitzbg/lumeshot/releases/tag/v0.1.13) is published. The
release workflow signed it with the Developer ID certificate, verified the team
identifier, and notarized and stapled the dmg. See the [release notes](releases/v0.1.13.md)
and [changelog](../CHANGELOG.md).

Validation: 474 tests pass on macOS 26.6.2 / Swift 6.3.3 and on CI. The download machinery
was separately confirmed against the live v0.1.12 release — correct version offered from a
simulated 0.1.11, all 7,314,487 bytes fetched matching the published asset, checksum
passed, quarantine attribute set. What remains unverified is the modal interaction itself:
no headless test can press a button, so the first real proof that Download works will be
updating from v0.1.13 to whatever follows it.
...
[v0.1.12](https://github.com/seitzbg/lumeshot/releases/tag/v0.1.12) is published. The
release workflow signed it with the Developer ID certificate, verified the team
identifier, and notarized and stapled the dmg. See the [release notes](releases/v0.1.12.md)
and [changelog](../CHANGELOG.md).

Validation: 470 tests pass on macOS 26.6.2 / Swift 6.3.3 and on CI, and the menu-bar
clipping was diagnosed by measuring the status item (22pt available against 53pt of
content). Not exercised by hand: the recordings destination picker, an actual upload to a
second destination, and the update download — this is the first release that can test the
download, from v0.1.11. The manual Gatekeeper, launch, capture and notification smokes
still date from v0.1.7.
...
[v0.1.11](https://github.com/seitzbg/lumeshot/releases/tag/v0.1.11) is published. The
release workflow signed it with the Developer ID certificate, verified the team
identifier, and notarized and stapled the dmg. See the [release notes](releases/v0.1.11.md)
and [changelog](../CHANGELOG.md).

Validation: 460 tests pass on macOS 26.6.2 / Swift 6.3.3 and on CI. The update download
path is covered by unit tests over the release payload and the checksum format, but has
never been exercised against a live release — this is the first build that can be, once
something newer than it exists. The manual Gatekeeper, launch, capture and notification
smokes below still date from v0.1.7.

- Removing an upload from History now clears its status everywhere. Deleting a failed
  upload used to leave the menu-bar `!` and the History banner reporting a failure for a
  row that no longer existed, because `UploadActivity` tracked results independently of
  the history store. Uploads now carry the id of the history row they belong to.
...
two uploaders configured. See v0.1.12.
