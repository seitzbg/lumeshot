...
**Goal:** Complete the Lumeshot annotation editor's v1 toolset — non-destructive crop, in-canvas text, translucent highlighter, Core Image blur/pixelate, and auto-numbering step badges — and split the editor's single "Done" into explicit **Copy / Save / Upload** actions wired into the local-first capture pipeline.
...
10. **Copy** action: click **Copy**; paste into another app → the **annotated image** appears; confirm **no file** was written to `~/Pictures/Lumeshot` for this capture (ephemeral).
11. **Save** action: new capture → annotate → **Save**; confirm a file **is** written to `~/Pictures/Lumeshot` and **no upload** happens.
...
- Unlimited undo/redo → M3a history reused; step renumber and crop-singleton are single atomic commits (T6, T7). ⚠️ Shipped bound is 50 (matches upstream `MaxAnnotationMementos`; README/porting-map already say "bounded to 50" — the spec's "unlimited" wording is the only mismatch, not a code change).
...
- **Undo stays bounded to 50** (spec says "unlimited"; matches upstream and M3a).
