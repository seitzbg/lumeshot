...
**Status:** v0.1.22 is released — capture, a full annotation editor, screen recording, multiple uploader backends, dedicated Settings, and a Developer ID signed + notarized `.dmg`. See **[docs/ROADMAP.md](docs/ROADMAP.md)** for the detailed status, pending manual smokes, and what's next. Local build: `swift build` (see [local development](docs/local-development.md)).

[Download v0.1.22](https://github.com/seitzbg/lumeshot/releases/tag/v0.1.22) · [Changelog](CHANGELOG.md)
...
HTTP response body or an SFTP server's status message, either of which can echo
an API key, a signed URL, a remote path or a deletion token back on an error.
The status code (HTTP or SSH_FX_*) and the app's own transport messages are kept.
